...
-- Hints library
local hints = require("lib.hints")

-- xrandr multi head support
local multihead = require("lib.multi_head")
...
tyrannical.tags = {
  {
    ...
    exec_once = { browser },
    class = { "Firefox", "Opera", "Chromium", "Aurora", "birdie", "Chromium-browser"},
  },
  {
    name = "2:dev",
    position = 2,
  ...
  },
  {
    ...
    layout = awful.layout.suit.tile,
    exec_once = { "gajim" },
    class = { "Kopete", "Pidgin", "gajim", "hipchat" }
  },
  {
    name = "4:doc",
    position = 4,
  ...
  },
  {
    ...
    floating = false,
    layout = awful.layout.suit.max,
    class = { "Eclipse", "NetBeans IDE", "jetbrains%-idea%", 
		    "jetbrains-idea"}
  },
  {
    name = "6:email",
...
tyrannical.properties.floating = {
  "MPlayer", "pinentry", ["jetbrains-idea"] = false
}


full_screen_apps = {"Firefox", "Opera", "Chromium", "Aurora", "Thunderbird", "evolution", "jetbrains-idea"}
...
awful.rules.rules = {
    ...
    { rule = { class = "gimp" },
      properties = { floating = true } },
    { rule = { class = "jetbrains-idea", type = "dialog" },
      properties = { floating = true, maximized_vertical = false, maximized_horizontal = false },
      callback = function (c)
        c.maximized_horizontal = false
        c.maximized_vertical = false
        awful.placement.centered(c, nil)
        awful.client.movetotag(c:tags()[mouse.screen][awful.tag.getidx()], c)
      end 
    },
...
function (widget, args)
 ...
 for k, v in pairs(args) do
   local dev = k:match("^{enp[^ ]+") or k:match("^{wlan[^ ]+")
   if dev ~= nil then
...
local networks = iwlist.scan_networks("wlan0")
if #networks > 0 then
  local msg = {}
...
local globalkeys = awful.util.table.join(
  ...
  awful.key({ modkey,           }, "Tab", function () getSequenceOfChars() end),
 ...
 -- awful.key({ "Shift" }, "space", function () mpc:toggle_play() mpc:update() end),
  ...
  awful.key({ }, "XF86AudioRaiseVolume", function () pulse_volume(1) end),
  awful.key({ }, "XF86AudioLowerVolume", function () pulse_volume(-1)end),
    ...
    function ()
      ...
      function (t) 
        naughty.notify{
          ...
          text  = "Running command "..t, 
          timeout = 1 }
          print(" Running  command "..t)
          local retval = awful.util.eval("return " .. t)
          print("Running command output " .. retval)
          --naughty.notify({title="retuning", text = "return value is "..retval, timeout=10})
      end, 
      nil,
  ...
  -- multihead
   awful.key({modkey}, "-", function () multihead.xrandr() end)
  )
...
function checkConfig()
  -- out = awful.util.pread("/usr/bin/awesome -k")
  out = awful.util.pread("awesome -k 2>&1")
  print("Error output " .. out)
  if string.find(out, "syntax OK") == nil then
    naughty.notify({preset = naughty.config.presets.critical,
                    title="ERROR", text = "return value is "..out, timeout=20})
    return false
  end
  return true
end
...
function takeBreak()
 ...
 testwidget:set_markup('<span style="text-decoration: bold;">a link</span>')
 mytimer:connect_signal("timeout", function() 
    interval = interval - 1
    testwidget:set_markup( '<span color="white">' .. interval .. ' mins </span> ') 
    mytimer:again()
    if interval == 0 then
        testwidget:set_markup( '<span color="red">!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1BReAK TImE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!</span> ') 
        mytimer:stop()
    end
  end) 
 mytimer:start()
...
testwidget:buttons(awful.util.table.join(
awful.button({ }, 1, function () 
                     quickNotify("echo Left mouse button pressed.") 
                     takeBreak()
                    end)
))
-- }}}
...
function getSequenceOfChars1(callBackfunc) 
  local seqOfChars = ""
   return function(mod, key, event) 
     ...
     if event == "press" then
       ...
       if key == "Return" then
         keygrabber.stop()
         callBackfunc(seqOfChars)
       else
         seqOfChars = seqOfChars .. key
         testwidget:set_text(seqOfChars)
       end
     end
     return true
   end
 ...
 function getSequenceOfChars() 
   function callbackcmd (cmd)
     print("Revcd cmd " .. cmd)
   end
   print("Starting grabber")
   afunc = getSequenceOfChars1(callbackcmd)
   keygrabber.run(afunc)
   print("Ending grabber")
   return "test"
 end
